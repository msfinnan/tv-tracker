...
export function ImportExport({ platforms, onImport }: ImportExportProps) {
  ...
  function handleExport() {
    ...
    setMessage({ type: 'success', text: 'Watchlist exported!' })
  }
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!validData) {
          setMessage({ type: 'error', text: 'Invalid file format. Expected a TV Tracker export.' })
          return
        }
        ...
        if (importMode === 'replace') {
          ...
          setMessage({ type: 'success', text: `Merged! ${newShows} new show(s) added.` })
        }
      } catch {
        setMessage({ type: 'error', text: 'Could not read file. Make sure it is valid JSON.' })
      }
    }
  ...
  return (
    <div className="import-export">
      <div className="import-export-actions">
        <button className="btn-export" onClick={handleExport} title="Download watchlist as JSON">
          Export
        </button>
        ...
        <div className="import-group">
          ...
          <button className="btn-import" onClick={handleImportClick} title="Import watchlist from JSON">
            Import
          </button>
        </div>
